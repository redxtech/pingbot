    ...
    flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { config, self', inputs', pkgs, system, ... }: {
        devenv.shells.default = {
          ...
          scripts.seal.exec = "kubeseal --format=yaml < ci/_secret.yaml > ci/secret.yaml";
          scripts.deploy.exec = "kubectl apply -k ci && kubectl rollout restart deployment/pingbot";

          # processes to run with `devenv up`
          processes.web.exec = "yarn start";
        };
      };
    };
}
